...
# Script zum Erstellen eines Ubuntu Noble Media-Center-Images für den Orange Pi 3B
...
IMAGE_NAME="ubuntu-noble-orangepi-3b-media-center.img"
...
export COMPATIBLE_FLAVORS=("media-center")  # Media-Center-Flavor
...
function config_image_hook__orangepi-3b() {
    ...
    # Media-Center-Umgebung installieren
    chroot "\${rootfs}" apt-get install -y kodi kodi-peripheral-joystick kodi-pvr-iptvsimple kodi-pvr-vuplus

    # Autostart für Kodi konfigurieren
    echo "[Unit]
...
[Install]
WantedBy=multi-user.target" > "\${rootfs}/etc/systemd/system/kodi.service"

    chroot "\${rootfs}" systemctl enable kodi
...
function build_image() {
    ...
    if ! ./build.sh --board=orangepi-3b --suite=noble --flavor=media-center; then
...
echo "Das Media-Center-Image wurde erfolgreich erstellt: ${IMAGE_PATH}"